...
router.get('/', async(req,res)=>{
    try{
        ...
        const request = await db.query(`Select estado, id_solicitud from tsolicitudes where (id_solicitante="${idUser}" and id_solicitado="${otherUser}") or (id_solicitante="${otherUser}" and id_solicitado="${idUser}") ORDER BY fecha_solicitud DESC`, { type: QueryTypes.SELECT })
        console.log(request)
        if(request.length>0) {
